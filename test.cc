...
#define STB_IMAGE_IMPLEMENTATATION
...
std::string PrintComponentType(int ty){
    ...
    else if(ty == TINYGLTF_COMPONRNT_TYPE_UNSIGNED_INT){
                return "UNSIGNED_INT";
    }
...
void DumpPrimitive(const Primitive& primitive , int indent)
{
    ...
    std::cout<< Indent(indent) << "mode     : " << PrintMode(primitive.mode)<< "("<< std::endl;
...
void Dump(const Scene &scene)
{
    ...
    std::cout<< "asset.version          : " <<scene.asset.premultipliedAlpha<<std::endl;
    std::cout<< "asset.profile.api      : " << scene.asset.version<<std::endl;
    std::cout<< "asset.profile.version  : " << scene.asset.profile_api<<std::endl;
    std::cout<< std::endl;
    std::cout<<"======Dump scene ========" << std::endl;
    std::cout<< "defaultScene: "<< scene.defaltScene<<std::endl;
    {
    ...
    }
    {
        ...
        for(;it != itEnd;it++){
                ...
                std::cout<< Indent(2) << "primitive( items= " << it->second.primitives.size() << "): " << std::endl;
    ...
    }
    {
        ...
        std::cout<<"accessors ( items =" << scene.accessors.size() << ")" << std::endl;
        for(;it!= itEnd ;it++)
        {
            std::cout<< Indent(1) << "name          : " << it->first<<std::endl;
            ...
            std::cout<< Indent(2) << "componentType : " << PrintComponentType(it->second.componentType)<<std::endl;
    ...
    {
        std::map<std::string,bufferView>::const_iterator it(scene.bufferView.begin());
        std::map<std::string,bufferView>::const_iterator itEnd(scene.bufferView.end());
        std::cout<< "bufferView ( items =" << scene.bufferView.size() << ")" << std::endl;
        for(;it!= itEnd;it++)
        {
          std::cout<< Indent(1) << "name            : " << it->first << std::endl;
          std::cout<< Indent(2) << "buffer          : " << it->second.buffer << std::endl;
          std::cout<< Indent(2) << "byteLength      : " << it->second.byteLength << std::endl;
          std::cout<< Indent(1) << "byteOffset      : " << it->second.byteOffset <<std::endl;            : " << it->second.byteOffset << std::endl;
